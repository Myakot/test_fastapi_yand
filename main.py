...
async def update_note(note_id: int, note: Note):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("UPDATE notes SET title = %s, content = %s WHERE id = %s", (note.title, note.content, note_id))
    conn.commit()
    cur.close()
    conn.close()
...
async def delete_note(note_id: int):
    ...
    if cur.rowcount == 0:
        return JSONResponse(content={"error": "Note not found"}, status_code=404, media_type="application/json")
    conn.commit()
    cur.close()
    conn.close()
...
def create_note_in_db(note):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("INSERT INTO notes (title, content) VALUES (%s, %s)", (note.title, note.content))
    conn.commit()
    cur.close()
    conn.close()
