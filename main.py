...
async def create_note_route(
    note: Note, current_user: User = Depends(get_current_user)
):
...
async def update_note(
    note_id: int, note: Note, current_user: User = Depends(get_current_user)
):
    # Spell check
...
def delete_all_notes(current_user: User = Depends(get_current_user)):
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "DELETE FROM notes WHERE username = %s",
...
async def delete_note(
    note_id: int, current_user: User = Depends(get_current_user)
):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute(
        "DELETE FROM notes WHERE id = %s AND username = %s",
        (note_id, current_user.username),
    )
    if cur.rowcount == 0:
